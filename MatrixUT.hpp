...
template <typename T> class MLMatrix {
  ...
  public:
    ...
    MLMatrix<T> addRow(const T, const T);
    MLMatrix<T> addCol(const T, const T);
    ...
    // DeepShift methods
    MLMatrix<int8_t> matRound(uint8_t = 10);
    MLMatrix<T> lShift(int8_t);
    MLMatrix<T> rShift(int8_t);
    MLMatrix<T> MultShift(MLMatrix<int8_t>);
