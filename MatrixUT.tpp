  ...
  for (size_t i = 0; i < rows; ++i) {
    ...
    for (size_t j = 0; j < cols; ++j) {
      if (std::is_floating_point<T>::value) { // float
        ...
        mat[i][j] = random(min, max); // random number between min and max-1 (both included)
      }
    }
  }
}
...
std::vector<T> MLMatrix<T>::diag_vec() {
  ...
  for (size_t i = 0; i < rows; ++i) result[i] = mat[i][i];
  return result;
}
...
{
  float mean = 0.0f;
  for (size_t j = 0; j < cols; ++j) mean += abs(this->mat[rowNumber][j]);
  mean /= float(cols);
  return mean;
}
...
{
  for (size_t j = 0; j < cols; ++j) this->mat[rowNumber][j] = T(0);
}

template <typename T>
void MLMatrix<T>::setZeroCol(const int colNumber)
{
  for (size_t i = 0; i < rows; ++i) this->mat[i][colNumber] = T(0);
}
...
{
  for (size_t j = 0; j < cols; ++j) this->mat[rowNumber][j] = value;
}

template <typename T>
void MLMatrix<T>::setCol(const int colNumber, const T value)
{
  for (size_t i = 0; i < rows; ++i) this->mat[i][colNumber] = value;
}
...
{
  for (size_t j = 0; j < cols; ++j) this->mat[rowNumber][j] = values(0, j);
}

template <typename T>
void MLMatrix<T>::setColMat(const int colNumber, MLMatrix<T> values)
{
  for (size_t i = 0; i < rows; ++i) this->mat[i][colNumber] = values(i, 0);
}
...
bool MLMatrix<T>::zeroRow(int rowNumber)
{
  ...
  for (size_t j = 0; j < cols; ++j) R(0,j) = mat[rowNumber][j];
  T valMax = R.max();
  T valMin = R.min();
...
bool MLMatrix<T>::zeroCol(int colNumber)
{
  ...
  for (size_t i = 0; i < rows; ++i) C(0,i) = mat[i][colNumber];
  T valMax = C.max();
  T valMin = C.min();
  isZero = (valMax == T(0)) && (valMin == T(0)) ? true : false;
  return isZero;
...
{
  uint16_t zero = 0;
  for (size_t j = 0; j < cols; ++j) if(mat[rowNumber][j] == T(0)) ++ zero;
  return zero;
}
...
{
  uint16_t zero = 0;
  for (size_t i = 0; i < rows; ++i) if(mat[i][colNumber] == T(0)) ++ zero;
  return zero;
}
...
MLMatrix<T> MLMatrix<T>::row(const uint16_t rowNumber)
{
  ...
  for (size_t j = 0; j < cols; ++j) result(0, j) = mat[rowNumber][j];
  return result;
}
...
MLMatrix<T> MLMatrix<T>::col(const uint16_t colNumber)
{
  ...
  for (size_t i = 0; i < rows; ++i) result(i, 0) = mat[i][colNumber];
  return result;
}
...
// Add a row at the end of a matrix, fills with random values in range
template<typename T>
MLMatrix<T> MLMatrix<T>::addRow(const T min, const T max)
{
  ++rows;
  mat.resize(rows);
  mat[rows - 1].resize(cols);
  for (unsigned j = 0; j < cols; ++j) {
    if (std::is_floating_point<T>::value) { // float
      float x = esp_random();
      x /= UINT32_MAX;
      mat[rows - 1][j] = min + x * (max - min);
    } else {
      mat[rows - 1][j] = random(min, max);
    }
  }
  return *this;
}

// Add a column at the end of a matrix, fills with random values in range
template<typename T>
MLMatrix<T> MLMatrix<T>::addCol(const T min, const T max)
{
  ++cols;
  for (unsigned i = 0; i < rows; ++i) {
    mat[i].resize(cols);
    if (std::is_floating_point<T>::value) { // float
      float x = esp_random();
      x /= UINT32_MAX;
      mat[i][cols - 1] = min + x * (max - min);
    } else {
      mat[i][cols - 1] = random(min, max);
    }
  }
  return *this;
}
...
// Make sure the input matrix has positive values !!!
// Adds 'shift' (default is 10) to the elements of the matrix
template<typename T>
MLMatrix<int8_t> MLMatrix<T>::matRound(uint8_t shift)
{
  MLMatrix<int8_t> result(rows, cols, 0);
  for (size_t i = 0; i < rows; ++i) {
    for (size_t j = 0; j < cols; ++j) {
      // if (mat[i][j] < T(-shift)) mat[i][j] = T(-shift); // clip!
      // if (mat[i][j] > T( shift)) mat[i][j] = T( shift);
      result(i,j) = round(mat[i][j] + shift);
    }
  }
  return result;
}

// Left shift the elements of a matrix
template<typename T>
MLMatrix<T> MLMatrix<T>::lShift(int8_t left)
{
  for (size_t i = 0; i < rows; ++i) {
    for (size_t j = 0; j < cols; ++j) {
      if (left > 0) mat[i][j] = mat[i][j] <<  left;
      else          mat[i][j] = mat[i][j] >> -left;
    }
  }
  return *this;
}

// Right shift the elements of a matrix
template<typename T>
MLMatrix<T> MLMatrix<T>::rShift(int8_t right)
{
  for (size_t i = 0; i < rows; ++i) {
    for (size_t j = 0; j < cols; ++j) {
      if (right > 0) mat[i][j] = mat[i][j] >>  right;
      else           mat[i][j] = mat[i][j] << -right;
    }
  }
  return *this;
}

// Multiply a vector by a shifting matrix: y = 2^P * x where P is an integer matrix
// Usage : y = x.MultShift(P);
template<typename T>
MLMatrix<T> MLMatrix<T>::MultShift(MLMatrix<int8_t> P)
{
  if (cols != P.get_rows()) {
    Serial.printf ("Matrix mult shift: incompatible dimensions (%d, %d).(%d, %d)\n",
      rows, cols, P.get_rows(), P.get_cols());
    while(1);
  }

  MLMatrix<T> result(rows, 0, 0);
  for (size_t i = 0; i < rows; ++i) {
    for (size_t j = 0; j < cols; ++j) {
      if (P(i,j) > 0) result(i,0) += mat[i][0] <<  P(i,j);
      else            result(i,0) += mat[i][0] >> -P(i,j);
    }
  }
  return result;
}
